    ...
    for _time in xrange(cross_times):
        ...
        for ((TRN, TEST), _t) in zip(super_set, xrange(len(super_set))):
            ...
            for design in designs:
                ...
                com = build_feedforward_committee(comsize, len(P[0]), netsize, 1, hidden_function = hidden_func, output_function = 'linear')
                ...
                #m.sendjob((count, _time, _t, design), train_committee, com, train_evolutionary, TRN_INPUTS, TRN_TARGETS, binary_target = 1, epochs = epochs, error_function = c_index_error, population_size = pop_size, mutation_chance = mutation_rate)
                #trn_errors, vald_errors, internal_sets = train_committee(com, train_evolutionary, TRN_INPUTS, TRN_TARGETS, binary_target = 1, epochs = epochs, error_function = c_index_error, population_size = pop_size, mutation_chance = mutation_rate)
    ...
    while(count > 0):
        ...
        if avg_val_c_index > best_com_val[_t]:
            ...
            best_test[_t] = tests[_c]
                ...
                #outputs = numpy.array([[com.risk_eval(inputs)] for inputs in TRN_INPUTS]) #Need double brackets for dimensions to be right for numpy
                #train_c_index = get_C_index(TRN_TARGETS, outputs)
    ...
    for _time in xrange(len(all_best)):
        ...
        for _t in best.keys():
            ...
            if len(TEST_INPUTS) > 0:
                outputs = numpy.array([[com.risk_eval(inputs)] for inputs in TEST_INPUTS]) #Need double brackets for dimensions to be right for numpy
                test_c_index = get_C_index(TEST_TARGETS, outputs)
            else:
                test_c_index = 0

            print('{trn}, {val}, {com_val}, {test}, {dsn}'.format(trn = best_avg_trn[_t], val = best_avg_val[_t], com_val = best_com_val[_t], test = test_c_index, dsn = best_design[_t]))
